...
def get_file_extension_list(src_path, output_file_name):
    extensions = set()
    find_command = "find " + src_path + " -type f -not -name '*\.cc' -not -name '*\.h'" + \
        " > " + output_file_name
    execute_command(find_command)
...
def get_source_name_from_slice(slice_path):
    if ".cc." in slice_path:
        source_path = slice_path.split(".cc.")[0] + ".cc"
    else:
...
def get_identifier_from_slice(slice_path):
    if ".cc." in slice_path:
        source_path, segment = slice_path.split(".cc.")
    else:
...
def remove_bracketed_content(s):
    ...
    return '::'.join([part for part in ''.join(result).split('::') if part.strip()])